                    ...
                    >
                        <template v-slot:images>
                            <div class="flex flex-col justify-center mt-3 h-32">
                                <i
                                    v-if="konten.logo == null"
                                    class="fas fa-image fa-4x text-gray-50"
                                ></i>
                                <img
                                    v-else
                                    :src="'/logo-client/' + konten.logo"
                                    alt=""
                        ...
                        >
                            ...
                            >
                                ...
                                >
                                    ...
                                    <span>Tambahkan Logo Client</span>
                                </div>
...
export default {
    ...
    methods: {
        ...
        createPOST() {
            ...
            form.append("nama_customer", this.nama_customer);
            form.append("description", this.description);
            form.append("alamat", this.alamat);
            form.append("logo", this.curenntImage);
            const config = {
                headers: {
        ...
        async getData() {
            return http
                .get("our-client")
                .then((respon) => (this.dbData = respon.data));
        },
        updateData(slugs) {
            this.modal = true;
        ...
        updateAction(slugs) {
            ...
            form.append("nama_customer", this.nama_customer);
            form.append("description", this.description);
            form.append("alamat", this.description);
            form.append("logo", this.curenntImage);
            const config = {
                headers: {
                    "Content-type": "multipart/form-data",
            ...
            return http
                .post("/our-client/" + slugs, form, config)
                .then((response) => {
                    // console.log(response.data.message);
                    this.message = response.data.message;
                    this.getData();
